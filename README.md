...
If Windows still resolves `python` or `pip` to global Python, use the helper scripts:
...
```cmd
.\.venv\Scripts\python.exe read_db.py
```
...
- If port `8080` is blocked on your machine, edit `run_backend.bat` and use another port like `8010`
